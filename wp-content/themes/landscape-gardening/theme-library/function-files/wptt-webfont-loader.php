...
if ( ! class_exists( 'Landscape_Gardening_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Landscape_Gardening_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $landscape_gardening_url The remote URL.
		 */
		public function __construct( $landscape_gardening_url = '' ) {
			$this->remote_url = $landscape_gardening_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $landscape_gardening_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $landscape_gardening_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $landscape_gardening_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $landscape_gardening_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $landscape_gardening_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $landscape_gardening_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $landscape_gardening_format = 'woff2' ) {
			$this->font_format = $landscape_gardening_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $landscape_gardening_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $landscape_gardening_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $landscape_gardening_url, '/' ) ) {
				$landscape_gardening_parsed_url = parse_url( $this->remote_url );
				return $landscape_gardening_parsed_url['scheme'] . '://' . $landscape_gardening_parsed_url['hostname'] . $landscape_gardening_url;
			}

			return $landscape_gardening_url;
		}
	}
}

if ( ! function_exists( 'landscape_gardening_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param string $landscape_gardening_url    The URL of the remote webfont.
	 * @param string $landscape_gardening_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function landscape_gardening_wptt_get_webfont_styles( $landscape_gardening_url, $landscape_gardening_format = 'woff2' ) {
		$landscape_gardening_font = new Landscape_Gardening_WPTT_WebFont_Loader( $landscape_gardening_url );
		$landscape_gardening_font->set_font_format( $landscape_gardening_format );
		return $landscape_gardening_font->get_styles();
	}
}

if ( ! function_exists( 'landscape_gardening_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function landscape_gardening_wptt_get_webfont_url( $landscape_gardening_url, $landscape_gardening_format = 'woff2' ) {
		$landscape_gardening_font = new Landscape_Gardening_WPTT_WebFont_Loader( $landscape_gardening_url );
		$landscape_gardening_font->set_font_format( $landscape_gardening_format );
		return $landscape_gardening_font->get_url();
	}
}