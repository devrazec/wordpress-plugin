...
<article>
	...
	<?php if( get_theme_mod( 'tour_travel_agent_single_post_date',true) != '' || get_theme_mod( 'tour_travel_agent_single_post_comment_no',true) != '' || get_theme_mod( 'tour_travel_agent_single_post_author',true) != '' || get_theme_mod( 'tour_travel_agent_single_post_time',true) != '') { ?>
        ...
        <span class="entry-date me-2 px-2"><i class="<?php echo esc_attr(get_theme_mod('tour_travel_agent_single_postdate_icon', 'far fa-calendar-alt me-1')); ?>"></i><?php echo esc_html( get_the_date() ); ?></span>
      <?php } ?>
      <?php if( get_theme_mod( 'tour_travel_agent_single_post_author',true) != '') { ?>
        <span class="entry-author me-2 px-2"><i class="<?php echo esc_attr(get_theme_mod('tour_travel_agent_single_postauthor_icon', 'fas fa-user me-1')); ?>"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span>
      <?php } ?>
      <?php if( get_theme_mod( 'tour_travel_agent_single_post_comment_no',true) != '') { ?>
        <span class="entry-comments me-2 px-2"><i class="<?php echo esc_attr(get_theme_mod('tour_travel_agent_single_postcomment_icon', 'fas fa-comments me-1')); ?>"></i> <?php comments_number( __('0 Comments','tour-travel-agent'), __('0 Comments','tour-travel-agent'), __('% Comments','tour-travel-agent') ); ?></span>
      <?php } ?>
      <?php if( get_theme_mod( 'tour_travel_agent_single_post_time',true) != '') { ?>
        <span class="entry-time me-2 px-2"><i class="<?php echo esc_attr(get_theme_mod('tour_travel_agent_single_posttime_icon', 'fas fa-clock me-1')); ?>"></i> <?php echo esc_html( get_the_time() ); ?></span>
      <?php }?>
		</div>
	<?php } ?>
