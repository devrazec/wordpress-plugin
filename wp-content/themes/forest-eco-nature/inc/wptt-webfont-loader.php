...
if ( ! class_exists( 'Forest_Eco_Nature_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Forest_Eco_Nature_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local forest_eco_nature_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $forest_eco_nature_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $forest_eco_nature_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$forest_eco_nature_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$forest_eco_nature_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$forest_eco_nature_font_family = trim( str_replace( array( "'", ';' ), '', $forest_eco_nature_font_family ) );
					$forest_eco_nature_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $forest_eco_nature_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $forest_eco_nature_font_family ] ) ) {
					$result[ $forest_eco_nature_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$forest_eco_nature_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$forest_eco_nature_font_family_url = $this->get_absolute_path( $forest_eco_nature_font_family_url );

					$result[ $forest_eco_nature_font_family ][] = $forest_eco_nature_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $forest_eco_nature_font_family ] = array_flip( array_flip( $result[ $forest_eco_nature_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'forest_eco_nature_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function forest_eco_nature_wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Forest_Eco_Nature_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'forest_eco_nature_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function forest_eco_nature_wptt_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Forest_Eco_Nature_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
